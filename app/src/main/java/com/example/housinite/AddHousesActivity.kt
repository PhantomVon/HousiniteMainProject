...
class AddHousesActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        edtDescription = findViewById(R.id.mEdtDescription)
    ...
    private fun uploadImage(){
        ...
        if (location.isEmpty()){
            ...
            edtDescription.setError("Please fill this input")
            ...
            if(filePath != null){
                ...
                val ref = storageReference?.child("houses/" + UUID.randomUUID().toString())
                progress.show()
                val uploadTask = ref?.putFile(filePath!!)!!.addOnCompleteListener{
                    progress.dismiss()
                    if (it.isSuccessful){
                        ref.downloadUrl.addOnSuccessListener{
                            var housesData = UploadHouses(location,bedrooms,description,rent,it.toString(),id)
                            var ref = FirebaseDatabase.getInstance().getReference().child("Houses/$id")
                            ref.setValue(housesData)
                            Toast.makeText(this, "House submitted successfully", Toast.LENGTH_SHORT).show()
                        }
                    }else{
                        Toast.makeText(this, "House submission failed", Toast.LENGTH_SHORT).show()
                    }
                }
